...
describe('central redaction and error sanitization', () => {
  it('redacts URLs, query credentials, and auth headers from strings', () => {
    const raw =
      'GET http://alice:secret@example.test/live?username=alice&password=secret Authorization: Bearer token';
  ...
  it('redacts sensitive object keys recursively', () => {
    const value = redactObject({ username: 'alice', nested: { token: 'secret', safe: 'ok' } });

    expect(value).toEqual({ username: '[REDACTED]', nested: { token: '[REDACTED]', safe: 'ok' } });
  });
