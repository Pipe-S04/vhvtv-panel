...
import { eq, and, ilike, count, inArray, asc } from 'drizzle-orm';
...
export async function channelRoutes(app: FastifyInstance, opts: { db: Database }): Promise<void> {
  ...
  app.get(
    ...
    async (request, reply) => {
      ...
      if (query.priority) conditions.push(eq(channels.priority, query.priority));
      if (query.monitorEnabled !== undefined)
        conditions.push(eq(channels.monitorEnabled, query.monitorEnabled));
  ...
  app.patch(
    ...
    async (request, reply) => {
      ...
      if (body.checkDurationSeconds !== undefined)
        updates.checkDurationSeconds = body.checkDurationSeconds;
